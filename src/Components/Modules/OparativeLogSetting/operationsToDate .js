...
const OperationsToDate = () => {
  ...
  const fetchSurgeons = async () => {
    try {
      const response = await AxiosAuthInstance.get(
        `${Constant.BASE_URL}/get-surgeons-none`
      );
  ...
  return (
    <>
      <div className="stylish relative bg-white w-100 px-5 2xl:px-10 rounded-md pt-5 pb-10 shadow-lg">
          ...
          <table className="operatives mt-16">
            ...
            <tbody>
              {Object.keys(monthlyDataSurgeons).map((name, index) => (
                name !== "NONE" && ( // Skip rendering if name is "NONE"
                  <tr key={index} className="text-left inter-bold">
                    ...
                    <td className="inter-bold">
                      <span className="text-[#B87D7D]">
                        {Array.from(
                          { length: 12 },
                          (_, i) => monthlyDataSurgeons[name]?.[i + 1]?.surgeries || 0
                        ).reduce((a, b) => a + b, 0)}
                      </span>
                      /
                      <span className="text-[#B87D7D]">
                        {Array.from(
                          { length: 12 },
                          (_, i) => monthlyDataAssistants[name]?.[i + 1]?.assistances || 0
                        ).reduce((a, b) => a + b, 0)}
                      </span>
                    </td>
                  </tr>
