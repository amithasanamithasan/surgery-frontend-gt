...
const SearchByDateDs = () => {
      ...
      const fetchScheduleForDate = async () => {
        try {
          const blockResponse = await AxiosAuthInstance.get(
            ...
            { params: { event_date: formattedDate } }
          );
          setBlock(blockResponse.data);
          ...
          const response = await AxiosAuthInstance.get(
            `${Constant.BASE_URL}/daily-schedule-search-by-date`,
            { params: { date: formattedDate } }
          );
          const formattedSchedule = response.data.surgeon_events;
          const timeList = response.data.times;
          const surgeons = response.data.surgeons;
          const surgeonsRole = response.data.surgeonsRole;
  ...
  const renderScheduleWithCollapsing = (time, surgeon_id) => {
    ...
    // Define the start and end times for AM and PM Initial slots
    const AM_START_INIT = AM_START;
    // const AM_END_INIT = "09:15";
    const PM_START_INIT = PM_START;
    // const PM_END_INIT = "15:15";

    if (AM_START_INIT) {
      // Modify time for End AM
      const [amHour, amMinute] = AM_START_INIT.split(":").map(Number);
      const amDate = new Date();
      amDate.setHours(amHour, amMinute, 0);
      amDate.setMinutes(amDate.getMinutes() + 15);
      var AM_END_INIT = amDate.toTimeString().slice(0, 5);
    } else {
      const AM_START_INIT = null;
    }
    if (PM_START_INIT) {
      // Modify time for End PM
      const [pmHour, pmMinute] = PM_START_INIT.split(":").map(Number);
      const pmDate = new Date();
      pmDate.setHours(pmHour, pmMinute, 0);
      pmDate.setMinutes(pmDate.getMinutes() + 15);
      var PM_END_INIT = pmDate.toTimeString().slice(0, 5);
    } 

    console.log(`AM Start: ${AM_START}, AM End: ${AM_END}`);
    console.log(`PM Start: ${PM_START}, PM End: ${PM_END}`);

    if (isAMFull && time >= AM_START_INIT && time <= AM_END_INIT) {
      const rowSpan = timeSlots.filter(t => t >= AM_START_INIT && t <= AM_END_INIT).length;
      const rowSpanAM = timeSlots.filter(t => t >= AM_START && t <= AM_END).length + 1;
      return (
        <td key={`${surgeon_id}-am`} rowSpan={rowSpan} className="px-5 2xl:px-0">
          <EventCard
            ...
            link="#"
            disable={true}
            height={`${rowSpanAM * 29}px`}
          />
        </td>
      );
    }

    // if (AM_START && AM_END && isAMFull && time >= AM_START && time <= AM_END) {
    ...
    //   );
    // }
    if (isPMFull && time >= PM_START_INIT && time <= PM_END_INIT) {
      const rowSpan = timeSlots.filter(t => t >= PM_START_INIT && t <= PM_END_INIT).length;
      const rowSpanPM = timeSlots.filter(t => t >= PM_START && t <= PM_END).length + 1;
      return (
        <td key={`${surgeon_id}-pm`} rowSpan={rowSpan} className="px-5 2xl:px-0">
          <EventCard
            ...
            link="#"
            disable={true}
            height={`${rowSpanPM * 29}px`}
          />
        </td>
      );
    }

    // if (PM_START && PM_END && isPMFull && time >= PM_START && time <= PM_END) {
      ...
      <div className="body-content bg-[#ECECEC]">
        ...
        <div className="w-90% lg:w-[90%] xl:w-[1200px] 2xl:w-[1400px] mx-auto py-3 relative pt-0 lg:pt-10 mb-10 2xl:px-0 px-5">
          <div className="bg-white w-100 px-2 2xl:px-10 rounded-md pt-5 pb-10 shadow-lg">
              ...
              <>
                <div style={{ overflowX: 'auto' }} ref={tableContainerRef}>
                  ...
                  <table className="surgery-schedule">
                    ...
                    <tbody>
                      {timeSlots.map((time) => {
                        const formattedTime = dayjs.utc(`2024-08-21T${time}`).utc().format("hh:mm A");
            ...
            <div class="three-items ps-2 2xl:ps-16 pe-[2%] mt-3">
              {block.map((item, index) => (
                <div class="bg-[#B4C6D9] rounded-md py-5 mt-2" key={index}>
                  <p class="text-center text-[10px] sm:text-[18px]">
                    <span class="inter-bold">{item.location} BLOCK:</span> {item.time}
                  </p>
                </div>
              ))}
            </div>
