...
const Messages = () => {
    ...
    return paginatedMessages.map((message) => (
      <div key={message.id} className="message-list left-0 right-0 m-auto mb-[16px]">
        <Link to={{
          ...
          onClick={() => { markMessageAsRead(message.id); }}
        >
          <div className={`h-[95px] w-full 2xl:w-[1325px] ${message.is_read ? 'read' : 'unread'} rounded-[10px] flex justify-between px-5 items-center`}>
            <h1 className="text-left inter-bold text-[18px] w-[250px] pt-1">
              {message.sender.name}
              <span className="text-[15px] text-[#CCCCCC] ml-2">
  ...
  const renderSendMessages = (messagesList) => {
    ...
    return paginatedMessages.map((message) => (
      <div key={message.id} className="message-list left-0 right-0 m-auto mb-[16px]">
        <Link to={{ pathname: `/sentbox/${message.id}`, state: { fromTab: activeTab } }}>
          <div className="h-[95px] w-full 2xl:w-[1325px] read rounded-[10px] flex justify-between px-5 items-center">
            <h1 className="text-left inter-bold text-[18px] w-[250px] pt-1">
              {message.sender.name}
              <span className="text-[15px] text-[#CCCCCC] ml-2">
                ({message.recipients.length} Recipients)
  ...
  const PaginationControls = () => {
    const totalItems = activeTab === 'inbox' ? messages.received.length : messages.sent.length;
    const totalPages = Math.ceil(totalItems / itemsPerPage);

    if (totalPages <= 1) return null;

    // Calculate page range for last 5 pages
    ...
    return (
      <div className="flex justify-center items-center gap-2 mt-8">
        <button
          ...
          className="px-4 py-2 rounded-lg bg-[#CCCCCC] inter-regular disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
          Previous
        </button>
        ...
        {visiblePages.map(page => (
          <button
            ...
            className={`px-4 py-2 rounded-lg ${currentPage === page
              ? 'bg-[#B4C6D9] inter-bold text-white hover:bg-[#B4C6D9]'
              : 'bg-[#CCCCCC] hover:bg-[#CCCCCC]'
              } transition-colors`}
          >
            {page}
          </button>
  ...
  return (
    <>
      ...
      <div className="w-[1200px] 2xl:w-[1400px] mx-auto py-3 relative pt-10">
        <div className="absolute flex w-[865px] 2xl:w-[1165px] justify-between top-[15px] left-0 right-0 m-auto">
          <div
            className={`inbox ${activeTab === "inbox" ? "msg-active" : ""} bg-[#CCCCCC] w-[350px] 2xl:w-[550px] border-2 rounded-lg py-3 text-center inter-bold text-[20px] cursor-pointer`}
            onClick={() => {
              setActiveTab("inbox");
              setCurrentPage(1);
            }}
          ...
          <div
            className={`to ${activeTab === "sent" ? "msg-active" : ""} bg-[#CCCCCC] w-[350px] 2xl:w-[550px] border-2 rounded-lg py-3 text-center inter-bold text-[20px] cursor-pointer`}
            onClick={() => {
              setActiveTab("sent");
              setCurrentPage(1);
            }}
        ...
        <div className="bg-white w-[1200px] 2xl:w-[1400px] px-10 rounded-md pt-16 pb-10 mb-10">
          {activeTab === "inbox"
