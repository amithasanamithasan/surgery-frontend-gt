...
import CallCalender from "./CallCalenderShow.js";
  ...
  return (
    <>
      <div className="bg-[#748BA2] content-center">
        <div className="global-centering relative w-[1300px] 2xl:w-[1400px] bg-white flex flex-wrap rounded-xl shadow-lg">
          {/* {localStorage.role != 3 && ( */}
            <CallCalender
          ...
          <div className="w-[94%] mx-auto mt-[55px] mb-[65px]">
            ...
            <div id="call-calender">
              ...
              <div className="week-calender" style={{display: "flex", justifyContent: "center"}}>
          ...
          <Link to="/block-calender" className="msg-submit rounded-md bg-[#B4C6D9] py-3 text-center inter-medium text-[24px]  w-[50%] absolute bottom-[-25px] left-0 right-0 m-auto"> View Block Time Calendar </Link>
        </div>
      </div>
      {/* <Preloads /> */}
    </>
...
const SurgeonSelect = ({ day, surgeons, vacations, entries, office }) => {
  ...
  const getSurgeonEntry = (callType, date) => {
    const entry = entries.find(entry =>
      dayjs.utc(entry.date).format("YYYY-MM-DD") == dayjs(date).format("YYYY-MM-DD")
    );
