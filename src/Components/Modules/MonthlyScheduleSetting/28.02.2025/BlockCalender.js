  ...
  return (
    <>
      <h3 className="text-[#ABABAB] inter-bold text-[15px]">
        Week {weekNumber} - <span className="text-xs text-[#ABABAB] "> {dayjs.utc(date).format("MMMM DD")} </span>
      </h3>
      <div className='flex'>
        <p className='flex justify-between w-[55px]'>{startHour}  : {startMinute}</p>
        <p class="mx-2">-</p>
        <p className='flex justify-end w-[55px]'>{endHour} : {endMinute} </p>
      </div>
    </>
  );
};
