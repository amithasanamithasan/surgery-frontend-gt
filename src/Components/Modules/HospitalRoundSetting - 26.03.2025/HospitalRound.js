  ...
  const PaginationControls = () => {
    ...
    return (
      <>
        {totalPages > 1 && (
          <>
            <div className="pagination-controls mt-4 flex justify-center items-center gap-2 pb-4">
              <button
                className="px-4 py-2 bg-[#B4C6D9] rounded-md disabled:opacity-50 hover:bg-[#657E98] hover:text-white transition-colors"
              ...
              {pageNumbers.map(number => (
                <button
                  key={number}
                  className={`px-4 py-2 min-w-[40px] rounded-md hover:bg-[#657E98] hover:text-white transition-colors ${currentPage === number
                    ? 'bg-[#657E98] text-white'
                    : 'bg-[#B4C6D9]'
                    }`}
