...
const Celender = ({ isCalendarOpen, setIsCalendarOpen }) => {
  ...
  const handleSearchDateSubmit = (e) => {
    ...
    navigate(`/hospital-round-search-date?date=${encodeURIComponent(formattedDate)}`);
  ...
  return (
    <>
      <button
        className="hp-cal absolute right-[20px] border-2 w-[175px] text-[18px] inter-medium h-[35px] rounded-md border-none bg-white  drop-shadow-2xl"
        onClick={() => setIsCalendarOpen(!isCalendarOpen)}
      >
        Select Date
      </button>
      {isCalendarOpen && (
        <div ref={calendarRef}  className="absolute right-[0px]  top-[0] z-10">
